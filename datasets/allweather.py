...
class AllWeather:
    ...
    def get_loaders(self, parse_patches=True, validation='snow'):
        ...
        train_dataset = AllWeatherDataset(os.path.join(self.config.data.data_dir, 'data', 'outdoor-rain'),
...
class AllWeatherDataset(torch.utils.data.Dataset):
    def __init__(self, dir, patch_size, n, transforms, filelist=None, parse_patches=True):
        ...
        with open(train_list) as f:
            ...
            gt_names = ["/".join(["/".join(i.split("/")[:-1]),i.split("/")[-1].split("_")[0] + ".png"]).strip().replace('input', 'gt') for i in input_names]
            "/".join(["/".join(input_string.split("/")[:-1]), input_string.split("/")[-1].split("_")[0] + ".png"])
